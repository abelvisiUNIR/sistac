...
from sistac.config import CVS_RAW, JOB_DESCRIPTIONS, GOLD_STANDARD_DIR
...
def seed_database():
    ...
    from sistac.config import ensure_dirs, EVAL_SETS
    ensure_dirs()
    ...
    if gt_path.exists() and (not train_ids_path.exists() or not test_ids_path.exists() or cvs_restaurados > 0 or cargos_restaurados > 0):
        try:
            from sistac.utils.split_corpus import stratified_split, save_split
