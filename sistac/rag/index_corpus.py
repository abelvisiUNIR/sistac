...
Uso:
    py -3 sistac/rag/index_corpus.py
    py -3 sistac/rag/index_corpus.py --dry-run   # ver stats sin subir
    py -3 sistac/rag/index_corpus.py --config c3  # indexa con PII anonimizada
...
from sistac.rag.chunking import chunk_text_tokens
from sistac.rag.pipeline import _upload_to_azure, _azure_headers, _upload_to_gcp
import requests
...
def load_corpus() -> tuple[dict[str, str], dict[str, str]]:
    ...
    if not CVS_RAW.exists():
        raise FileNotFoundError(
            ...
            "  py -3 sistac/data/synthetic_corpus_generator.py"
        )
...
def verify_index_count() -> None:
    ...
    if VECTORSTORE_PROVIDER == "google":
        from sistac.config import GCP_PROJECT_ID, GCP_LOCATION, GCP_DATA_STORE_ID
        try:
        ...
        from sistac.config import AZURE_SEARCH_ENDPOINT
        url = (
...
if __name__ == "__main__":
    ...
    if not args.dry_run:
        ...
        print("Próximo paso:")
        print("  py -3 sistac/experiments/orquestador_c0_c3.py")
