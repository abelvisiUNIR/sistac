...
CÓMO EJECUTAR:
    python sistac/experiments/orquestador_c0_c3.py
...
from thesis_experiments.evaluation.efficacy_metrics import efficacy_report
from thesis_experiments.evaluation.efficiency_metrics import efficiency_report
from thesis_experiments.evaluation.fairness_metrics import fairness_report
...
def main() -> None:
    ...
    # H3 — Equidad (C2 vs C3)
    # H3 — Equidad (C2 vs C3)
    h3_c2 = compute_h3_metrics(c2_results, "C2", group_attr="group_gender")
    h3_c3 = compute_h3_metrics(c3_results, "C3", group_attr="group_gender")
    h3_age_c2 = compute_h3_metrics(c2_results, "C2", group_attr="group_age")
    h3_age_c3 = compute_h3_metrics(c3_results, "C3", group_attr="group_age")
    ...
    # Guardar tablas H3
    h3_headers = ["Configuración", "DIR (género)", "SPD (género)", "DIR (edad)", "SPD (edad)", "H3 aceptada (DIR ≥ 0.80)"]
    h3_rows = [
        ["C2 (LLM + RAG)",
         f"{h3_c2.get('dir', 'N/A'):.3f}" if h3_c2 else "N/A",
         f"{h3_c2.get('spd', 'N/A'):.3f}" if h3_c2 else "N/A",
         f"{h3_age_c2.get('dir', 'N/A'):.3f}" if h3_age_c2 else "N/A",
         f"{h3_age_c2.get('spd', 'N/A'):.3f}" if h3_age_c2 else "N/A",
         "Sí" if h3_c2 and h3_c2.get("dir", 0) >= 0.80 and h3_age_c2 and h3_age_c2.get("dir", 0) >= 0.80 else "No"],
        ["C3 (LLM + RAG + PII)",
         f"{h3_c3.get('dir', 'N/A'):.3f}" if h3_c3 else "N/A",
         f"{h3_c3.get('spd', 'N/A'):.3f}" if h3_c3 else "N/A",
         f"{h3_age_c3.get('dir', 'N/A'):.3f}" if h3_age_c3 else "N/A",
         f"{h3_age_c3.get('spd', 'N/A'):.3f}" if h3_age_c3 else "N/A",
         "Sí" if h3_c3 and h3_c3.get("dir", 0) >= 0.80 and h3_age_c3 and h3_age_c3.get("dir", 0) >= 0.80 else "No"],
    ]
    save_word_table(
        headers=h3_headers,
        rows=h3_rows,
        ...
        note=(
            ...
            "Grupos evaluados: género (F vs M) y edad (36-58 vs 23-35). "
