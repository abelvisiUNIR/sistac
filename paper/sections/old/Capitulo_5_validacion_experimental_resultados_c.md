...
La mediana del tiempo de cribado manual (estimada de forma indirecta mediante imputación basada en la tasa promedio de lectura operativa de la organización) fue de 661.8 segundos por candidato. Es importante destacar que el tiempo de la condición C0 no constituye una medición directa en tiempo real de los evaluadores durante la sesión de etiquetado, sino una estimación imputada a partir de los tiempos promedio de cribado manual registrados históricamente por la organización para perfiles similares. Esto permite contar con una línea base comparativa sin alterar el flujo de trabajo del panel. Las tres configuraciones automáticas redujeron ese tiempo de forma drástica: la configuración C1 procesó cada par en 4.5 segundos (factor 147.8×), C2 en 6.8 segundos (96.7×) y C3 en 19.6 segundos (33.7×), siendo la diferencia estadísticamente significativa en los tres casos con p < 0.0001. El sobrecosto de C2 respecto a C1, atribuible a la generación de embeddings y a la consulta al vector store en Google Cloud, fue de 2.3 segundos; el sobrecosto adicional de C3 respecto a C2, correspondiente a la ejecución local del módulo SistacAnonymizer con spaCy y Presidio, fue de 12.8 segundos, lo que explica el IQR considerablemente mayor de C3 (7.9 s) respecto al de C1 y C2. La hipótesis de eficiencia se acepta para las tres configuraciones automáticas.
